...
import { getMyRole, getAdminMonitoring, type AdminMonitorRow } from "@/lib/data/queries"
...
export default async function AdminPage() {
  ...
  const admins = await getAdminMonitoring()

  const total  = admins.length
  const active = admins.filter((a) => statusOf(a.last_sign_in_at) === "active").length
  const never  = admins.filter((a) => statusOf(a.last_sign_in_at) === "never").length

  return (
    <div className="p-6 space-y-6 max-w-5xl">
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <ShieldCheck size={24} className="text-brand-500" /> Monitoring Admin
        </h1>
        <p className="text-sm text-[var(--color-muted)] mt-0.5">
          Pantau admin (kepala keluarga) yang aktif & kapan terakhir mereka menggunakan sistem. Hanya untuk super admin.
        </p>
      </div>

      {/* ── Kartu ringkasan ────────────────────────────────── */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <StatCard icon={<Users size={18} />}     label="Total Admin"            value={total.toLocaleString("id-ID")} />
        <StatCard icon={<UserCheck size={18} />}  label={`Aktif (${ACTIVE_WINDOW_DAYS} hari)`} value={active.toLocaleString("id-ID")} />
        <StatCard icon={<UserX size={18} />}      label="Belum pernah login"    value={never.toLocaleString("id-ID")} />
      </div>
